...
torch.set_float32_matmul_precision("high") # set the precision of the matmul to high

# load the data from the binary files, which we downloaded from the downloader.py script
...
# Apply MNIST normalization (mean=0.1307, std=0.3081), the values we used is from the original MNIST dataset
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features, num_classes):
        ...
        # a simple 2-layer MLP, use relu as the activation function
        self.fc1 = nn.Linear(in_features, hidden_features)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(hidden_features, num_classes)

    def forward(self, x):
        x = x.reshape(batch_size, 28 * 28) # flatten the image
        x = self.fc1(x)
        x = self.relu(x)
        x = self.fc2(x)
        return x
...
# the shape of the output matches the number of classes
model = MLP(in_features=784, hidden_features=1024, num_classes=10).to("cuda")

# Weight Initialization: Apply custom He initialization to match NumPy/C implementations
with torch.no_grad():
